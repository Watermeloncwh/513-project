...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($name_err) && empty($address_err) && empty($salary_err) && empty($username_err)&& empty($password_err)){
        ...
        $sql = "UPDATE vip SET name=?, address=?, salary=? , username=?, password=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssisss", $param_name, $param_address, $param_salary, $param_username, $param_password, $param_id);
